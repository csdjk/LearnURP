Shader "LcL/VAT/VAT3_RBD2"
{
    ...
    Properties
    {
        ...
        _UniformScale ("Uniform Scale", Range(0,100)) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void ApplyVATTransform(inout float3 positionOS, inout float3 normalOS, inout float4 tangentOS, Attributes input)
            {
                ...
                // 5. 获取缩放值
                float3 scaleValue = _UniformScale;
                ...
                // 然后应用旋转
                pivotVec = RotateWithQuaternion(pivotVec, quat_de);

                // 最后应用位置偏移（位置偏移也需要缩放）
                positionOS = (posOffset.xyz * scaleValue) + pivotVec ;

                // 8. 旋转法线和切线（注意：法线不受缩放影响，但需要考虑非均匀缩放的情况）
                ...
                // 如果是非均匀缩放，需要对法线进行额外处理
                if (abs(scaleValue.x - scaleValue.y) > 0.001 || abs(scaleValue.y - scaleValue.z) > 0.001)
                {
                    // 非均匀缩放时，法线需要用逆转置矩阵变换
                    float3 invScale = 1.0 / scaleValue;
                    normalOS = normalize(normalOS * invScale);
                }
            }
