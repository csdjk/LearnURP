Shader "LcL/DepthRim"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float miHoYoCalculateDepthRimLight(float2 screenPos, float rimWidth, float depth)
            {
                float depthSelf = Linear01Depth(depth, _ZBufferParams);

                float rimFactor = depthSelf * _ProjectionParams.z + 3.0;
                // rimWidth = rimWidth * 0.01;

                float2 offsetCoord = screenPos + float2(rimWidth / rimFactor, 0.0);
                float offsetDepth = SAMPLE_TEXTURE2D_LOD(_CameraDepthTexture, sampler_CameraDepthTexture, offsetCoord, 0).r;
                offsetDepth = Linear01Depth(offsetDepth, _ZBufferParams);

                float depthDiff = offsetDepth - depthSelf;
                depthDiff = pow(max(depthDiff, 0.000001), 0.015);
                // depthDiff = smoothstep(0.0, 1.0, (depthDiff - 0.82) * 12.5);
                depthDiff = SmoothValue(_RimLightThreshold, _RimLightSmoothness, (depthDiff - 0.82) * 12.5);
                return depthDiff;
            }
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half rimLight = miHoYoCalculateDepthRimLight(screenPos, _RimLightWidth, input.positionCS.z);
                color.rgb += rimLight;
                return rimLight;
