    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                // output.tangentWS = TransformObjectToWorldDir(input.tangentOS,true);
                ...
                // float3 lightDirOS = mul(UNITY_MATRIX_I_M, float4(L, 0)).xyz;
                // lightDirOS = normalize(lightDirOS);

                // output.viewDirWS.xy = lightDirOS.yz;
            ...
            half4 frag(Varyings input, half facing : VFACE) : SV_Target
            {
                ...
                // color.r = bool(step(0.5, _Test0)) ? _Test0 : 0;

                // bool2 u_xlatb26 = (float4(0.0, 0.0, 0.1, 0.1) < _Test0.xxxx).xz;
                // color.r = u_xlatb26.x ? 1 : 0;
                // color.g = u_xlatb26.y ? 1 : 0;

                // ;                float2 v = step( _Test0.xxxx,float4(0.0, 0.0, 0.1, 0.1)).xz;
                // color.rg = v;

                // bool2 u_xlatb26 = (float4(0.0, 0.0, 0.1, 0.1) < _Test0.xxxx).xz;
                // color.rg = u_xlatb26? 1 : 0;


                // color.r = lerp(1, _Test0, _Test1);
                // color.r = lerp(1, _Test0*_Test1, _Test1);
