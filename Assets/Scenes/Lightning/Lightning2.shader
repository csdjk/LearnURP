	...
	SubShader
	{
		...
		Pass
		{
			...
			Varyings vert(Attributes IN)
			{
				...
				float3 tangent = cross(IN.tangent.xyz, directionToCamera);
				float4 offset = float4(normalize(tangent) * IN.tangent.w, 0);
				// OUT.positionCS = UnityObjectToClipPos(worldPos + (offset));
				VertexPositionInputs positionInputs = GetVertexPositionInputs(worldPos + (offset));
				OUT.positionCS = positionInputs.positionCS;
