...
namespace LcLTools
{
    public class GpuSpineBaker
    {
        ...
        string m_MeshPath;
        MeshFilter m_SelectSkinnedMeshRenderer;

        public MeshFilter SelectMeshRenderer
        {
        ...
        Vector3[] Vertices => SelectMeshRenderer.sharedMesh.vertices;
        Vector3[] Normals => SelectMeshRenderer.sharedMesh.normals;

        Vector4[] Tangents => SelectMeshRenderer.sharedMesh.tangents;
        ...
        Matrix4x4[] BindPoses
        {
            get
            {
                var bones = new Matrix4x4[1];
                bones[0] = SelectMeshRenderer.transform.localToWorldMatrix;
                return bones;
            }
        }
        // BoneWeight[] BoneWeights => SelectMeshRenderer.sharedMesh.boneWeights;
        ...
        {
            // 创建新的Mesh
            var mesh = UnityEngine.Object.Instantiate(SelectMeshRenderer.sharedMesh);

            // 将骨骼索引和权重保存到UV1和UV2
            var boneIndexs = Vertices.Select(v => new Vector4(0, 0, 0, 0)).ToArray();
